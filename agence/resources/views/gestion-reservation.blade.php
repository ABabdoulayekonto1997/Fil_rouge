            ...
            <div class="bg-white overflow-hidden shadow-lg rounded-lg">
                <div class="p-6">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-bold text-[#0C4069]">Gestion des Réservations</h2>
                        <div class="flex space-x-4">
                            <div class="relative">
                                <input type="text" placeholder="Rechercher..." class="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-[#D88F42] focus:border-[#D88F42]">
                                <svg class="absolute left-3 top-2.5 h-5 w-5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                                </svg>
                            </div>
                            <select class="border border-gray-300 rounded-lg px-4 py-2 focus:ring-[#D88F42] focus:border-[#D88F42]">
                    ...
                    <div class="overflow-x-auto bg-white rounded-lg shadow">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-[#0C4069] uppercase tracking-wider">Client</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-[#0C4069] uppercase tracking-wider">Destination</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-[#0C4069] uppercase tracking-wider">Date de départ</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-[#0C4069] uppercase tracking-wider">Statut</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-[#0C4069] uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10 bg-[#0C4069] rounded-full flex items-center justify-center">
                                                <span class="text-white font-medium">{{ substr($reservation->utilisateur->name, 0, 1) }}</span>
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $reservation->voyage ? $reservation->voyage->destination : 'N/A' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="flex items-center space-x-3">
                                            @if($reservation->statut === 'en_attente')
                                                <form action="{{ route('reservations.confirm', $reservation->id) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="bg-green-500 text-white px-3 py-1 rounded-md hover:bg-green-600 transition-colors duration-200">
                                                        Confirmer
                                                    </button>
                                                </form>
                                            @endif
                                            <button class="text-[#0C4069] hover:text-[#D88F42] transition-colors duration-200">
